...
ggplot(data = time_mem_both_2D, aes(x = n_states, y = time, group = method)) + geom_line(aes(color = method)) +
  ...
  theme_bw(base_size = 16) +
    ...
    scale_color_discrete(name = "Approaches", labels = c("Dynamics array", "Tranditional cohort trace"))
...
ggplot(data = time_mem_both_2D, aes(x = n_states, y = time, group = method)) + geom_line(aes(color = method)) +
  ggtitle("Computation time simulation analysis (adjusted y-axis)") +
  xlab("Number of health states") + ylab("Time (sec)") +
...
p <- plot_ly(x = time_mem_trace$n_states, y = time_mem_trace$n_t, z = time_mem_trace$time, type = 'scatter3d', mode = 'lines', name = 'Traditional cohort trace', connectgaps=FALSE) %>% layout(scene = list(yaxis = axy), title = "Computational time by number of health state and cycles") %>%
layout(scene = list(
       xaxis = list(title = "Number of health states"),
       yaxis = list(title = "Number of cycles"),
       zaxis = list(title = "Time (sec)"))) %>% layout(scene=scene)

p %>% add_trace(x = time_mem_array$n_states, y = time_mem_array$n_t, z = time_mem_array$time, type = 'scatter3d', mode = 'lines', name = 'Dynamics array', connectgaps = FALSE) 
```
...
```{r, echo = FALSE, eval = TRUE, warning = FALSE, cache = TRUE, message = FALSE}
options(scipen = 999)
ggplot(data = time_mem_both_2D, aes(x = n_states, y = memory/1000000, group = method)) + geom_line(aes(color = method)) +
  ggtitle("Computation memory of comparison") +
  xlab("Number of health states") + ylab("Memory (MB)") +
  theme_bw(base_size = 16) +
    scale_y_continuous(breaks = number_ticks(7)) +
    scale_x_continuous(breaks = number_ticks(6)) +
  scale_color_discrete(name = "Approaches", labels = c("Dynamics array", "Tranditional cohort trace"))
...
p1 <- plot_ly(x = time_mem_trace$n_states, y = time_mem_trace$n_t, z = time_mem_trace$memory, type = 'scatter3d', mode = 'lines', name = 'Traditional cohort trace', connectgaps = FALSE) %>% layout(scene = list(yaxis = axy), title = "Computational memory by number of states and cycles") %>%
layout(scene = list(
      xaxis = list(title = "Number of health states"),
      yaxis = list(title = "Numer of cycles"),
      zaxis = list(title = "Memory (MB)"))) %>% layout(scene=scene)

p1 %>% add_trace(x = time_mem_array$n_states, y = time_mem_array$n_t, z = time_mem_array$memory, type = 'scatter3d', mode = 'lines', name = 'Dynamics array', connectgaps = FALSE) 
```
...
```{r, echo = FALSE, eval = TRUE, warning = FALSE, cache = TRUE, message = FALSE}
library(reshape2)
time_mem_both_2D$memory <- time_mem_both_2D$memory/1000000
time_mem_both_2D_new <- melt(time_mem_both_2D, id.vars = c("method", "n_states", "n_t"))
