...
#### Equation 9
By using the `colSums` function, summing the values in each column of `a.A`, we can do this for all points in time. By transposing this in turn, we get the traditional cohort trace `m.M`. We name this matrix, `m.M_A`, to indicate it is generated via the dynamics array approach and to make it possible to compare it with the previously generated `m.M`. When you only do the dynamics array approach you can simply call it `m.M`. 
```{r, message = FALSE, warning = FALSE}
...
By now you know how to run a state-transition cohort model using the dynamics array approach and how to interpret or summarize the results. In this section, we demonstrate how to apply state and transition rewards. We start by initiating and filling two matrices for both costs and effects. On the diagonal are the costs for staying one cycle in that state, while the costs off the diagonal are the costs for staying one cycle in that state plus the transition cost associated with that transition.
...
# get the transition probability p.HH and p.HS from array A for each cycle
p.HH[t] <- a.A["H", "H", t + 1] / sum(a.A["H", , t + 1])  
p.HS[t] <- a.A["H", "S", t + 1] / sum(a.A["H", , t + 1])  
...
v.CI <- cumsum(n.newCases) / a.A["H", "H", 1] 
# create a dataframe with cycles and cumulative incidence
df.CI <- as.data.frame(cbind(0:n.t, v.CI)) 
colnames(df.CI) <- c("cycle", "CI") # name the columns of the dataframe
```


```{r cumulative-incidence, fig.width = 7, fig.height = 4, fig.align = 'center', fig.pos = "h", echo = FALSE, fig.cap = 'Cumulative incidence of first time getting sick ', message = FALSE, warning = FALSE} 
