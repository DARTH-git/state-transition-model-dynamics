...
As the number of health states increases, when running 1320 cycles, the run time of the cohort trace approach increases almost exponentially while that of the array approach stays invariant, and significantly less as shown in Figure \ref{fig:2D-time}.

![Computation time of cohort trace appraoch v.s array approach \label{fig:2D-time}](../figs/2D_time.png)
...
At 1320 cycles, as the number of health states increases, the storage of both approaches increases (see Figure \ref{fig:2D-memory}. When the number of health states is less than approximately 52, the cohort trace approach takes up less storage. But when the number of health states is above approximately 52, it takes up more storage. 

![Computation memory of cohort trace appraoch v.s array approach when running 1320 cycles \label{fig:2D-memory}](../figs/2D_memory.png)
