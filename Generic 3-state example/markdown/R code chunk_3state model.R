...
|           **Parameter**            |  **R name** |   **Value**   |
  ...
  | - Disease onset (H to S)           | `p.HS`      |  0.17         |
  | - Recovery (S to H)                | `p.SH`      |  0.35         |
  | Annual mortality                   |             |               |
  | - All-cause (H to D)               | `p.HD`      |  0.05         |
  | - Disease specific (S to D)        | `p.SD`      |  0.10         |
  | Annual costs                       |             |               |
...
In this section, we specify the staring age of the cohort, the number of cycles, the names of the health states and the initial state vector. The parameters described in the table are combined in a data frame using the `f.generate_init_params` function. 
```{r, eval = TRUE, echo = TRUE, message = FALSE, warning = FALSE}
...
By using the `colSums` function, summing the values in each column of `a.A`, we can do this for all points in time. By transposing this in turn, we get the traditional cohort trace `m.M`. We name this matrix, `m.M_A`, to indicate it is generated via the dynamics array approach and to make it possible toe compare it with the previously generated `m.M`. When you only do the dynamics array approch you can simply call it `m.M`. 
```{r, message = FALSE, warning = FALSE}
...
#### Equation 10 
In this section we create outcome arrays, `a.O`, one for costs, called `a.O_costs`, and one for effects, called `a.O_effects`. These arrays show the costs and QALYs generated with each transition at each cycle. By iteratively element-wise multiplication of the reward matrices, `m.R_costs` and `m.R_effects`, with array `a.A` we can fill the outcome arrays.
```{r, message = FALSE, warning = FALSE}
...
#### Equation 11 
The final step is to calculated to the total costs and QALYs. We start by calculating the expected cost and QALYs per cycle. These values, stored in the vectors `v.Costs` and `v.QALYs`, in turn we sum these values. This gives us the total expected cost (`TC`) and QALYs (`TE`). 
```{r, message = FALSE, warning = FALSE}
