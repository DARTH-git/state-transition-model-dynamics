...
# This is extra code to try to make the figures
...
data_relative$Approach <- "Approach comparison"
...
ggplot(data_relative, aes(x = n_states, time)) +
  ...
  scale_x_continuous(breaks = number_ticks(6)) +
  xlab("Number of health states") + 
  ylab("Relative speedup: dynamic / traditional") +
      theme_bw(base_size = 16) + 
      theme(legend.position = "bottom")

#ggsave("../figs/2D_time_relative.png")
...
ggplot(data_relative, aes(x = n_states, memory)) +
  ...
  scale_x_continuous(breaks = number_ticks(6)) +
  xlab("Number of health states") + 
  ylab("Relative memory usage: dynamic / traditional") + #\u0394 is the unicode for Delta
      theme_bw(base_size = 16) + 
      theme(legend.position = "bottom")


data_relative_combined <- melt(data_relative, id.vars = c("Approach", "n_states", "n_t"))

#ggsave("../figs/2D_memory_relative.png")


# Use the facet_wrap to make a figure
ggplot(data_relative_combined, aes(x = n_states, value), group = variable) +
  geom_point(color = DARTHgreen) + 
  geom_line(color = DARTHgreen) +
  scale_y_continuous(breaks = number_ticks(7)) +
  scale_x_continuous(breaks = number_ticks(6)) +
  xlab("Number of health states") + 
  ylab("Relative: dynamic / traditional") +  
  facet_wrap(~ variable, scales ="free_y") + 
    geom_hline(aes(yintercept = 1), color = DARTHgray) +
      theme_bw(base_size = 16) + 
      theme(legend.position = "bottom") +
...
names(data_relative)[names(data_relative) == "time"] <- "Relative time: dynamic/traditional"
names(data_relative)[names(data_relative) == "memory"] <- "Relative memory: dynamic/traditional"

data_relative_combined_new_names <- melt(data_relative, id.vars = c("Approach", "n_states", "n_t"))


data_combined <- rbind(time_mem_both_2D_new, data_relative_combined_new_names)
...
ggsave("../figs/time_memory_comparison.png")
```
